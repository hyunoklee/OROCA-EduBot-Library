...
void Adafruit_VL53L0X::update(void)
{
	FixPoint1616_t  LimitCheckCurrent;	
    rangingTest(&measure, false); // pass in 'true' to get debug data printout!
	VL53L0X_GetLimitCheckCurrent( pMyDevice, VL53L0X_CHECKENABLE_RANGE_IGNORE_THRESHOLD, &LimitCheckCurrent );
	measure.RangIgnoreThreshold = (float)LimitCheckCurrent / 65536.0;
	...
	if( measure.RangIgnoreThreshold < 1 ){
	   measure.RangeMilliMeter = 2000 ;
	}
 
	Serial.print("Left Distance (mm): "); Serial.print(measure.RangeMilliMeter);
	Serial.print(", RangIgnoreThreshold"); Serial.print(measure.RangIgnoreThreshold);
	Serial.print(", State"); Serial.println(measure.RangeStatus);

	distance_mm = measure.RangeMilliMeter ;

}

void Adafruit_VL53L0X::end( void)
{
}
